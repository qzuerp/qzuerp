...
<input type="hidden" id="MPSEVRAKTYPE" name="MPSEVRAKTYPE">
    ...
    function evrakIslemleriSwal(islemTipi) {
        ...
        if (islemTipi === "evrakKaydet") {
            return new Promise((resolve) => {
                Swal.fire({
                    title: "Bu MPS'i ne için oluşturuyorsun?",
                    showDenyButton: true,
                    confirmButtonText: "Üretim",
                    denyButtonText: "Fason",
                    confirmButtonColor: "#28a745",
                    denyButtonColor: "#007bff",
                    icon: "question"
                }).then((result) => {
                    if (result.isConfirmed) {
                        $("#MPSEVRAKTYPE").val("U"); // Üretim
                        resolve("uretim");
                    } 
                    else if (result.isDenied) {
                        $("#MPSEVRAKTYPE").val("F"); // Fason
                        resolve("fason");
                    } 
                    else resolve(false);
                });
