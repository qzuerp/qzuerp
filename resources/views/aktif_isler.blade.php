...
<head>
  ...
  <style>
    ...
    *::after {
    ...
    .sc-total .summary-icon {
      background: #eff6ff;
      color: #3b82f6;
    }

    .sc-total .summary-value {
      color: #1e40af;
    }

    .sc-uretim .summary-icon {
      background: #ecfdf5;
      color: #10b981;
    }

    .sc-uretim .summary-value {
      color: #047857;
    }

    .sc-ayar .summary-icon {
      background: #fffbeb;
      color: #f59e0b;
    }

    .sc-ayar .summary-value {
      color: #b45309;
    }

    .sc-durus .summary-icon {
      background: #fef2f2;
      color: #ef4444;
    }

    .sc-durus .summary-value {
      color: #b91c1c;
    }
    ...
    /* Status-specific card backgrounds & borders */
    .job-card.card-uretim {
      background: linear-gradient(135deg, #f0fdf4 0%, #ffffff 60%);
      border-left-color: #10b981;
      box-shadow: 0 2px 12px rgba(16, 185, 129, 0.10);
    }

    .job-card.card-uretim:hover {
  ...
  <div class="main-container">
    ...
    <div class="filter-bar">
      <div class="filter-group">
        ...
        <button class="filter-btn" data-filter="uretim" onclick="filterCards('uretim', this)">
          <i class="fa-solid fa-play" style="color:#10b981;font-size:10px"></i> Üretim <span
            class="badge">{{ $uretimSay }}</span>
        </button>
        <button class="filter-btn" data-filter="ayar" onclick="filterCards('ayar', this)">
          <i class="fa-solid fa-wrench" style="color:#f59e0b;font-size:10px"></i> Ayar <span
            class="badge">{{ $ayarSay }}</span>
        </button>
        <button class="filter-btn" data-filter="durus" onclick="filterCards('durus', this)">
          <i class="fa-solid fa-pause" style="color:#ef4444;font-size:10px"></i> Duruş <span
            class="badge">{{ $durusSay }}</span>
        </button>
      </div>
      <div class="search-box">
        <i class="fa-solid fa-magnifying-glass"></i>
        <input type="text" id="searchInput" placeholder="Stok kodu, tezgah veya iş no ara..."
    ...
    <div class="cards-grid" id="cardsGrid">
      @foreach ($isler as $index => $is)
        @php
          ...
          if ($is->BITIS_SAATI == null && $is->BITIS_TARIHI == null) {
            if ($is->ISLEM_TURU == 'A') {
              ...
              $statusType = 'ayar';
              $pillClass = 'pill-ayar';
              $accentClass = 'accent-ayar';
              $progressClass = 'progress-fill-ayar';
            } elseif ($is->ISLEM_TURU == 'U') {
              $durum = 'Üretim';
              $statusType = 'uretim';
              $pillClass = 'pill-uretim';
              $accentClass = 'accent-uretim';
              $progressClass = 'progress-fill-uretim';
            } elseif ($is->ISLEM_TURU == 'D') {
              $durum = 'Duruş';
              $statusType = 'durus';
              $pillClass = 'pill-durus';
              $accentClass = 'accent-durus';
              $progressClass = 'progress-fill-durus';
            }
          }
        ...
        <div class="job-card card-{{ $statusType }}" data-status="{{ $statusType }}"
          ...
          <div class="card-body">
            ...
            <div class="progress-section">
              <div class="progress-header">
                ...
                <span class="progress-values">{{ floor($gerceklesen) }} / {{ floor($planlanan) }} <span
                    style="color:#94a3b8;font-weight:500">({{ $yuzde }}%)</span></span>
              </div>
