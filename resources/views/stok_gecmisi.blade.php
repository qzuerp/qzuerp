...
@section('content')
    ...
    <div class="content-wrapper">
        ...
        <div class="content">
            <div class="stok-wrapper">
                ...
                {{-- SONUÇLAR --}}
                <div class="card-compact results-compact p-3" id="resultsCard">
                    <div class="row g-2 mb-3" id="ozetKutular" style="display:none">
                        <div class="col-md-4">
                            <div class="card-compact ozet-card ozet-ilk">
                                <div class="card-body-compact">
                                    <div class="ozet-title"><i class="far fa-calendar"></i> Dönem</div>
                                    <div id="donemOzet" class="ozet-value"></div>
                                </div>
                            </div>
                        </div>

                        <div class="col-md-4">
                            <div class="card-compact ozet-card">
                                <div class="card-body-compact">
                                    <div class="ozet-title"><i class="fas fa-chart-pie"></i> Dağılım</div>
                                    <div id="dagilimOzet" class="ozet-value"></div>
                                </div>
                            </div>
                        </div>

                        <div class="col-md-4">
                            <div class="card-compact ozet-card ozet-son">
                            <div class="card-body-compact">
                                <div class="ozet-title"><i class="fas fa-bolt"></i> Son Aktivite</div>
                                <div id="sonAktiviteOzet" class="ozet-value"></div>
                            </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <table class="table table-sm table-hover" id="gecmisTable">
                            <thead>
                                <tr>
                                    ...
                                    <th><i class="fas fa-external-link-alt"></i> Git</th>
                                </tr>
                            </thead>
                            <tbody></tbody>
                        </table>
    ...
    <script>
        function ozetGuncelle(data) {
            if (!data || data.length === 0) {
                $('#ozetKutular').hide();
                return;
            }

            // data TARIH DESC geliyor varsayımıyla
            const son = data[0];
            const ilkTarih = data[data.length - 1].tarih;
            const sonTarih = son.tarih;

            // kaynak sayımları
            const sayac = {};
            data.forEach(r => {
                const k = (r.kaynak || 'DİĞER').toUpperCase();
                sayac[k] = (sayac[k] || 0) + 1;
            });

            const rozetler = Object.entries(sayac)
                .sort((a,b) => b[1]-a[1])
                .map(([k,v]) => `<span class="badge bg-secondary me-1">${k}: ${v}</span>`)
                .join(' ');

            $('#donemOzet').html(`<div>${ilkTarih} → ${sonTarih}</div><div class="text-muted">Kayıtların kapsadığı dönem</div>`);
            $('#dagilimOzet').html(`<div>${rozetler}</div><div class="text-muted">Kaynağa göre dağılım</div>`);
            $('#sonAktiviteOzet').html(`<div>${son.tarih}</div><div class="text-muted">${son.kaynak} – ${son.no}</div>`);
            ...
            let gecmisTable = $('#gecmisTable').DataTable({
                ...
                columns: [
                    ...
                    { data: 'aciklama', width: '58%' },
                    {
                        ...
                        width: '20%',
                        render: function (data) {
                            return `<a href="${data}" target="_blank" class="btn btn-primary btn-sm"><i class="fas fa-external-link-alt"></i> Git</a>`;
                        }
                    }
                ]
            });
