...
@php
	...
	$GK1_veri = DB::table($database.'gecoust')->where('EVRAKNO','DEPOGK1')->get();
	$GK2_veri = DB::table($database.'gecoust')->where('EVRAKNO','DEPOGK2')->get();
	$GK3_veri = DB::table($database.'gecoust')->where('EVRAKNO','DEPOGK3')->get();
	$GK4_veri = DB::table($database.'gecoust')->where('EVRAKNO','DEPOGK4')->get();
	$GK5_veri = DB::table($database.'gecoust')->where('EVRAKNO','DEPOGK5')->get();
	$GK6_veri = DB::table($database.'gecoust')->where('EVRAKNO','DEPOGK6')->get();
	$GK7_veri = DB::table($database.'gecoust')->where('EVRAKNO','DEPOGK7')->get();
	$GK8_veri = DB::table($database.'gecoust')->where('EVRAKNO','DEPOGK8')->get();
	$GK9_veri = DB::table($database.'gecoust')->where('EVRAKNO','DEPOGK9')->get();
	$GK10_veri = DB::table($database.'gecoust')->where('EVRAKNO','DEPOGK10')->get();
...
<div class="content-wrapper">
  ...
  @include('layout.util.logModal',['EVRAKTYPE' => 'GDEF00','EVRAKNO'=>@$kart_veri->EVRAKNO])

  <section class="content">
    <form class="form-horizontal" action="gdef00_islemler" method="POST" name="verilerForm" id="verilerForm">
      @csrf
						...
						<table id="example2" class="table table-hover text-center" data-page-length="10">
				          ...
				          // Veritabanı dinamik olarak belirleniyor
						    	...
						    	if(isset($_GET['GK_10_E'])) {$GK_1_E = TRIM($_GET['GK_10_E']);}
									...
									$sql_sorgu = 'SELECT * FROM ' . $database . 'gdef00 WHERE 1 = 1';
									if(Trim($KOD_B) <> ''){
								...
								<table id="evrakSuzTable" class="table table-hover text-center" data-page-length="10" style="font-size: 0.8em">
									...
									<tbody>
										...
										foreach ($evraklar as $key => $suzVeri) {
												...
												echo "<td>"."<a class='btn btn-info' href='kart_depo?ID=".$suzVeri->id."'><i class='fa fa-chevron-circle-right' style='color: white'></i></a>"."</td>";
