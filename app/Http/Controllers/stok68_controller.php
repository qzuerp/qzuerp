...
class stok68_controller extends Controller
{
  ...
  public function islemler(Request $request)
  {
    ...
    switch($islem_turu) {
      ...
      case 'kart_sil':
        ...
        $KONTROL_VERILERI = DB::table($firma . 'stok68t')->where('EVRAKNO', $EVRAKNO)->get();
      ...
      case 'kart_olustur':
        ...
        for ($i = 0; $i < $satir_say; $i++) {
          ...
          DB::table($firma.'stok10a')->insert([
            ...
            'SRNUM' => $SRNUM,
            'TRNUM' => $TRNUM[$i],
            'KOD' => $R_YMAMULCODE->R_YMAMULKODU ?? $KOD[$i],
            'STOK_ADI' => $AD->AD ?? $STOK_ADI[$i],
            'LOTNUMBER' => $LOTNUMBER[$i],
            'SERINO' => $SERINO[$i],
            'SF_MIKTAR' => $SF_MIKTAR[$i],
            'SF_SF_UNIT' => $SF_SF_UNIT[$i],
      ...
      case 'kart_duzenle':
        ...
        for ($i = 0; $i < $satir_say; $i++) {
          ...
          $KAYITLI_SF = DB::table($firma . 'stok68t')
        ...
        foreach ($deleteTRNUMS as $key => $deleteTRNUM) {
            $KONTROL_VERI = DB::table($firma . 'stok68t')
                ->where('EVRAKNO', $EVRAKNO)
  ...
  public function fason_getir(Request $request)
  {
      ...
      $res = DB::table($firma.'stok63e as e')
          ->join($firma.'stok63t as t', 'e.EVRAKNO', '=', 't.EVRAKNO')
          ->whereExists(function ($query) use ($firma, $DEPO) {
              $query->select(DB::raw(1))
                    ->from($firma.'stok10a as a')
                    ->whereRaw('a.KOD = t.KOD')
                    ->whereRaw('a.LOTNUMBER = t.LOTNUMBER')
                    ->where('a.AMBCODE', $DEPO);
          })
          ->where('e.CARIHESAPCODE', $URETICI)
          ->where('e.AMBCODE', $DEPO)
          ->get();
