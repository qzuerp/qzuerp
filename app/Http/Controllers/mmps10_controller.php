    ...
    switch ($islem_turu) {
      ...
      case 'kart_olustur':
        ...
        DB::table($firma . 'mmps10e')->insert([
          ...
          'SF_TOPLAMMIKTAR' => $SF_PAKETSAYISI * $SF_PAKETICERIGI,
    ...
    for ($i = 0; $i < count($KOD); $i++) {
      ...
      if (DB::table($firma . 'bomu01e')->where('MAMULCODE', $KOD[$i])->exists()) {
        ...
        for ($j = 0; $j < count($table); $j++) {
          $ELAST_TRNUM = str_pad($j+1, 6, "0", STR_PAD_LEFT);
          $row = $table[$j];
          $TRNUM = str_pad($j+1, 6, "0", STR_PAD_LEFT);
        ...
        DB::table($firma.'mmps10e')->where('EVRAKNO',$NEXT_EVRAKNO)->update([
          'LAST_TRNUM' => $ELAST_TRNUM
        ]);
      }
